...
import { X, Clock, Users, ChevronDown, ChevronRight, CheckCircle2, Circle, Download } from 'lucide-react';
...
export default function PracticePreview({ patterns, onClose }: PracticePreviewProps) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('practice-checks');
    if (saved) {
        ...
        <div className="modal-header">
          <div>
            ...
            <div className="preview-meta">
              ...
              <span>进度 {completedCount}/{patterns.length} ({Math.round(completedCount / Math.max(patterns.length, 1) * 100)}%)</span>
            </div>
          </div>
